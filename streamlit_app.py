...
# ===========================
# PLACEHOLDERS PARA COLEGAS
# ===========================

def calcular_prioridade_non_preemptive():
    return "Em desenvolvimento pelo Integrante 1"

def calcular_prioridade_preemptive():
    return "Em desenvolvimento pelo Integrante 2"
...
menu = ["M/M/1", "M/M/s", "M/M/1/K", "M/M/s/K", "M/M/1/N", "M/M/s/N", "M/G/1", "Prioridade Non-Preemptive", "Prioridade Preemptive"]
escolha = st.sidebar.selectbox("Escolha o Modelo", menu)

st.header(f"Configuração do Modelo {escolha}")

if "Prioridade" in escolha:
    st.info(f"🚧 Este módulo ({escolha}) está sendo desenvolvido pelos seus colegas de grupo.")
else:
    with st.expander("Parâmetros de Entrada", expanded=True):
        ...
        if "n" in escolha.lower() and "non" not in escolha.lower():
            extra_params['N'] = st.number_input("População (N)", min_value=1, value=50)
        if "g/1" in escolha.lower():
            extra_params['sigma'] = st.number_input("Desvio Padrão do Serviço (σ)", min_value=0.0, value=0.0, step=0.1, help="Use 0 para tempo constante (M/D/1)")
    ...
    elif escolha == "M/G/1": res = calcular_mg1(lamb, mu, extra_params['sigma'])
